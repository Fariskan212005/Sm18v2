...
from info import *
from tamilgram_extras import *
from database.verify import verify_user_data, verify_user
from utils import *
...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [
            [
                InlineKeyboardButton(text=SB_TEXT1, url=SB_URL1)
            ]
            ]
    ...
    if len(message.command) != 2:
        buttons = [[
            InlineKeyboardButton('⛨ 𝗔𝗗𝗗 𝗠𝗘 𝗔𝗦 𝗔𝗗𝗠𝗜𝗡 ⛨', url=f'http://t.me/{temp.U_NAME}?startgroup=true')
            ],[
            InlineKeyboardButton(text=SB_TEXT1, url=SB_URL1)
            ],[
            InlineKeyboardButton(text=SB_TEXT2, url=SB_URL2)
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
    ...
    if data.split("-", 1)[0] == "VERIFY":
        ...
        if key == user["vtoken"]:
            ...
            await verify_user_data(user_id, {"time": times})
            return await message.reply("Verified Successfully.\n\nNow You Can Use Me ❤️.")
        elif len(key) == 20 :
            return await message.reply("This Verify Link Expired Or Invalid.\n\n<b>Verify Again</b>\n\nClick /start to Get Verification Link")

    try:
        access = await checking_access(user_id)
        if access is None:
            pass
        else:
            vtoken = await generate_random_string(20)
            await verify_user_data(user_id, {"vtoken": vtoken})
            await message.reply(f"𝗬𝗼𝘂 𝗡𝗲𝗲𝗱 𝗧𝗼 𝗩𝗲𝗿𝗶𝗳𝘆 𝗬𝗼𝘂 𝗔𝗿𝗲 𝗔 𝗛𝘂𝗺𝗮𝗻. 𝗦𝗼 𝗬𝗼𝘂 𝗛𝗮𝘃𝗲 𝗧𝗼 𝗢𝗽𝗲𝗻 𝗧𝗵𝗶𝘀 𝗟𝗶𝗻𝗸 𝗔𝗻𝗱 𝗩𝗲𝗿𝗶𝗳𝘆.\n\nLINK : {await get_shortlink(await get_shortlink2(await get_shortlink3(f'https://t.me/{temp.U_NAME}?start=VERIFY-{vtoken}')))}\n\n👌 HOW TO VERIFY : {VERIFY_VIDEO}")
            return None
...
async def help(client, message):
        buttons = [[
            InlineKeyboardButton('𝖬𝖠𝖭𝖴𝖠𝖫 𝖥𝖨𝖫𝖳𝖤𝖱', callback_data='manuelfilter')
            ],[
            InlineKeyboardButton('𝖠𝖴𝖳𝖮 𝖥𝖨𝖫𝖳𝖤𝖱', callback_data='autofilter')
            ],[
            InlineKeyboardButton('𝖢𝖮𝖭𝖭𝖤𝖢𝖳𝖨𝖮𝖭', callback_data='coct')
            ],[
            InlineKeyboardButton('𝖤𝖷𝖳𝖱𝖠 𝖥𝖤𝖠𝖳𝖴𝖱𝖤𝖲', callback_data='extra')
            ],[
...
@Client.on_message(filters.command('qstop') & filters.user(ADMINS))
async def stopbot(bot, message):
    sys.exit()
